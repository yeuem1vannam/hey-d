...
if echo "$output" | grep -qi "hey-d:brainstorming\|hey-d:lets-get-started\|Available skills"; then
    echo "  [PASS] find_skills tool discovered hey-d skills"
else
...
# Test 3: Test use_skill with hey-d: prefix
echo ""
echo "Test 3: Testing use_skill with hey-d: prefix..."
echo "  Running opencode with hey-d:brainstorming skill..."

output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load hey-d:brainstorming and tell me the first few lines of what you received." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "brainstorming\|Launching skill\|skill.*loaded"; then
    echo "  [PASS] use_skill loaded hey-d:brainstorming skill"
else
    echo "  [FAIL] use_skill did not load hey-d:brainstorming correctly"
    echo "  Output was:"
    echo "$output" | head -50
    exit 1
fi
