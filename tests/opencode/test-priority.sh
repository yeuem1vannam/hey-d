...
# Verifies that skills are resolved with correct priority: project > personal > hey-d
...
# 1. Create in hey-d location (lowest priority)
mkdir -p "$HOME/.config/opencode/hey-d/skills/priority-test"
cat > "$HOME/.config/opencode/hey-d/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
...
if [ -f "$HOME/.config/opencode/hey-d/skills/priority-test/SKILL.md" ]; then
...
# Test 2: Test that personal overrides hey-d
echo ""
echo "Test 2: Testing personal > hey-d priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides hey-d)"
    ...
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|hey-d" | head -10
fi

# Test 3: Test that project overrides both personal and hey-d
echo ""
echo "Test 3: Testing project > personal > hey-d priority..."
...
# Test 4: Test explicit hey-d: prefix bypasses priority
echo ""
echo "Test 4: Testing hey-d: prefix forces hey-d version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load hey-d:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_SUPERPOWERS_VERSION"; then
    echo "  [PASS] hey-d: prefix correctly forces hey-d version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] hey-d: prefix did not force hey-d version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
