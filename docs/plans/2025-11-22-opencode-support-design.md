...
Add full hey-d support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port hey-d to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/hey-d-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/hey-d.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/hey-d/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/hey-d-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "hey-d:brainstorming")')
  }),
...
1. **Inject using-hey-d content**
   - Full content of the using-hey-d skill
...
export const SuperpowersPlugin = async ({ client, directory, $ }) => {
  const heyDDir = path.join(process.env.HOME, '.config/opencode/hey-d');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingSuperpowers = await readSkill('using-hey-d');
...
├── .codex/
│   ├── hey-d-codex        # UPDATED: Use skills-core
│   ├── hey-d-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── hey-d.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/hey-d-codex`
...
2. Update `.codex/hey-d-codex` to use shared core
...
1. Create `.opencode/plugin/hey-d.js`
